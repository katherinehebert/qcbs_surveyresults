...
impacts_df$Var1 = factor(impacts_df$Var1, levels = rev(c("Fortement positif / Strongly positive",
                                                     "Positif / Positive", 
                                                     "Neutre / Neutral",
                                                     "Negative / Négatif",
                                                     "Fortement négatif / Strongly negative",
                                                     "Ne s'applique pas / Does not apply")))
# do stacked percent barchart
ggplot(impacts_df, aes(fill=Var1, y=Freq, x=category)) + 
  geom_bar(position="fill", stat="identity") +
  labs(y = "Proportion of student members (%)", 
       x = "QCBS activities /\nActivités du CSBQ") +
  scale_fill_manual(values = rev(c(nice_palette[1:3], "lightgrey")))+
  theme_classic() +
...
```{r wordcloud, echo = FALSE, fig.width = 10, fig.height = 7, message=FALSE}
# Q18 qcbs in 3 words ----
...
# remove "inaccessible"
r18_freqs_df = r18_freqs_df[-grep("inaccessible", r18_freqs_df[,1]),]
